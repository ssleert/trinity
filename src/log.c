...
static const char* LogVerbosityStringsColored[] = {
    "sfome on the swag",
    "[\033[0;34mTRACE\033[1;0m]",
    "[\033[0;32mINFO\033[1;0m] ",
    "[\033[0;33mWARN\033[1;0m] ",
    "[\033[0;31mERROR\033[1;0m]",
    "[\033[0;31mFATAL\033[1;0m]",
};
    ...
    const char Filename[], const char Fmt[], ...)
{
    ...
    if (LogMaxVerbosity == LOG_VERBOSITY_None || Verbosity == LOG_VERBOSITY_None) {
        return;
    }
    ...
    bool LocalLogColored = (Stream == stdout || Stream == stderr) ? LogColored : false;

    strftime(TimeBuffer, sizeof(TimeBuffer),
        (LogAddDate) ? "%Y-%m-%d %H:%M:%S" : "%H:%M:%S", LogCurrentTime());

    StringPointer += sprintf(StringPointer, "%s %s %s:%zu: ", TimeBuffer,
